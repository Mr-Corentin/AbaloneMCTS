...
def move_single_marble(board: chex.Array, 
    ...
    """
    Déplace une seule bille dans une direction donnée.
    ...
    """
    # Obtenir la valeur (couleur) de la position de départ
    pos_idx = position + radius
    start_value = board[pos_idx[0], pos_idx[1], pos_idx[2]]
    
    # Vérifier que la position contient une bille
    has_marble = (start_value == 1) | (start_value == -1)
    ...
    new_board = new_board.at[new_pos_idx[0], new_pos_idx[1], new_pos_idx[2]].set(
        jnp.where(success, start_value, dest_value)
    )
...
def move_group_parallel(board: chex.Array, 
    ...
    # Vérifier que toutes les nouvelles positions sont valides et vides
    ...
    def update_board(current_board):
        ...
        # Remplir les nouvelles positions
        return for_old_pos.at[
            ...
            idx_mask,
            jnp.where(success, piece_color, board[
                board_positions[:, 0],
                board_positions[:, 1],
                board_positions[:, 2]
            ]),
...
def move_group_inline(board: chex.Array, 
    ...
    # 3. Obtenir la couleur des billes qui bougent
    start_positions = valid_positions + radius
    moving_color = board[start_positions[0, 0], start_positions[0, 1], start_positions[0, 2]]
   ...
   # print("push in bounds", push_in_bounds)
    ...
    #print("push_content", push_contents)
    is_opposing = push_contents == -moving_color
    has_opposing = is_opposing[0]  # Juste la première position
    n_opposing = jnp.argmin(is_opposing)  # Nombre de billes adverses consécutives

    no_friendly_behind = (n_opposing == 0) | (push_contents[n_opposing] != moving_color)
    ...
    is_moving_piece = (actual_board_state == moving_color) & positions_mask  # Bille du mouvement
...
# Valider les positions cibles
    ...
    is_blocking_friend_ahead = content_ahead == moving_color  # On compare avec moving_color plutôt que 1


    # Combiner avec la logique existante
    #movement_valid = jnp.all(is_valid_position) & ~is_blocking_friend_ahead
   # destinations_valid = jnp.all(is_valid_position) & ~is_blocking_friend_ahead
    destinations_valid = jnp.all(is_valid_position) & ~is_blocking_friend_ahead & ((content_ahead == 0) | (has_opposing & can_push))
    #destinations_valid = (content_ahead == 0) | (has_opposing & can_push)
    # Sélectionner le bon cas
    ...
    for i in range(positions.shape[0]):
        # On crée un nouveau board qui aurait la position i remplie
        temp_board = updated_board.at[
            board_positions[i, 0],
            board_positions[i, 1],
            board_positions[i, 2]
